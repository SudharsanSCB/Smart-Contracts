...
//     }
    ...
    //mapping(address => uint256) public studentStructs;
...
uint256[] public printitem;
    ...
    function getAllRoyalty() public view returns(address payable[] memory){
     
      return participants;
    }

    function mintWithTokenURI(address payable to,  string memory tokenURI) public payable  returns (bool) {
        require(msg.value > 0, "Ether value must be greater than 0");
        nftIds.increment();
        uint256 ids = nftIds.current();
        _mint(to, ids);
        _setTokenURI(ids, tokenURI);
        items[ids].tokenOwner = to;
        // address payable receiver = 0x3764d8Fde1b0222877FC5427A56C25EE3029CdB6 ; //0xeA3FCB45f8E27F4fc30675e760d1109f7911B579; // Replace with the actual receiver's address
...
function setTokenState(uint256 ids, bool isEnabled) public  {
    // for (uint256 i = 0; i < ids.length; i++) {
      if(isEnabled == true){
        require((msg.sender == items[ids].tokenOwner), "TokenOwner: caller does not the owner role");
        items[ids].state = TokenState.ForSale;
      } else {
        items[ids].state = TokenState.Pending;
      }
    // }
     emit BatchForSale(ids, items[ids].metaId);
  }

  function setTokenPrice(uint256 ids, uint256 setPrice) public  {
    // for (uint256 i = 0; i < ids.length; i++) {
      require((msg.sender == items[ids].tokenOwner), "TokenOwner: caller does not the owner role");
      items[ids].price = setPrice;
    // }
  ...
  function buyThing(uint256 _tokenId) public payable returns (bool) {
    ...
    if(items[_tokenId].price >= 0) {
        ...
        // uint256 split = withFee.div(45);
        // uint256 splits = withFee.div(5);
        // feeAddress.transfer(split);        //creator
        // maker.transfer(fee);               //contract address        
        // participants.push(items[_tokenId].tokenOwner);        
        // for (uint i=0; i<participants.length; i++) {
        //     uint256 divid = participants.length;
        //     uint256 rewards = splits.div(divid);
        //     //studentStructs[participants[i]].transfer(split);
        //     // address payable parti = participants[i];
        //     participants[i].transfer(rewards);
        //     //address payable f = participants[i].makePayable();
        //     //payable(participants[i]).transfer(rewards);
        // }    
    }
        
    _transferFrom(maker, msg.sender, _tokenId);
    items[_tokenId].state = TokenState.Sold;
    items[_tokenId].tokenOwner = msg.sender;
